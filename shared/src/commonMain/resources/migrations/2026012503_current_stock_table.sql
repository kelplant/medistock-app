...
CREATE INDEX IF NOT EXISTS idx_current_stock_product ON current_stock_table(product_id);
CREATE INDEX IF NOT EXISTS idx_current_stock_site ON current_stock_table(site_id);
CREATE INDEX IF NOT EXISTS idx_current_stock_quantity ON current_stock_table(quantity);
CREATE INDEX IF NOT EXISTS idx_current_stock_client_id ON current_stock_table(client_id);

-- Initialize current_stock from existing stock_movements
INSERT INTO current_stock_table (id, product_id, site_id, quantity, last_updated_at)
SELECT
