...
/// Uses a pure Swift implementation compatible with OpenBSD BCrypt
final class PasswordHasher {
    ...
    func verifyPassword(_ plainPassword: String, storedPassword: String) -> Bool {
        if isHashed(storedPassword) {
            return BCryptSwift.verify(password: plainPassword, hash: storedPassword)
        } else {
    ...
    func hashPassword(_ password: String, cost: Int = 12) -> String? {
        BCryptSwift.hash(password: password, cost: cost)
    }
}

// MARK: - BCrypt Implementation (OpenBSD compatible)

/// Pure Swift BCrypt implementation with correct Blowfish S-boxes
private enum BCryptSwift {

    // BCrypt base64 alphabet (different from standard base64)
    static let base64Chars = Array("./ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789")

    // "OrpheanBeholderScryDoubt" magic constant
...
// MARK: - Blowfish State with correct S-boxes from pi
...
private struct BlowfishState {
    ...
    init() {
        P = BlowfishConstants.P
        S0 = BlowfishConstants.S0
        S1 = BlowfishConstants.S1
        S2 = BlowfishConstants.S2
        S3 = BlowfishConstants.S3
    }
...
// MARK: - Blowfish Constants (from pi digits - OpenBSD standard)
